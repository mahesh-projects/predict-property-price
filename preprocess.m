...
% Split Train, Validation, Test set into 70 - 20 -  10 proportion

% set probabilityTrain = 0.7
% set probabilityVal = 0.2
% set probabilityTest = 0.1
...
trainData = fullData(indexVec, :);

% Save remaining data 
dataRem = fullData(~indexVec, :);
...
indexVec_rem(1:round(pVal*N)) = true;

% Split remaining data into 20% validation data
...
fprintf('Validation Dataset size %f \n', size(valData));
fprintf('Test Dataset size %f \n', size(testData));
